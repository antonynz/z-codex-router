...
3. If upgrade fails after a transaction begins, the control plane restores its just-created backup before returning the error. Use `rollback` only when the user explicitly asks to restore the latest completed state. Do not replace a stable profile with a candidate profile: candidates are disabled and unevaluated by design.
4. Stop on conflict, incompatible profile/runtime, permission, or path errors and report the stable code without modifying files manually.
